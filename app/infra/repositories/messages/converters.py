...
def convert_message_to_document(message: Message) -> dict:
    return {"oid": message.oid, "text": message.text.as_generic_type(), "created_at": message.created_at}
...
def convert_chat_entity_to_document(chat: Chat) -> dict:
    return {
        ...
        "message": [convert_message_to_document(message) for message in chat.messages],
    }
...
def convert_chat_document_to_entity(chat_document: Mapping[str, Any]) -> Chat:
    return Chat(
        ...
        messages={
            ...
            for message_document in chat_document["message"]
        },
    )
